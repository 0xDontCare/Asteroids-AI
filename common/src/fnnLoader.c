...
{
  // checking validity of arguments (only bias matrices can be ignored if unused)
  ...
  for (uint32_t i = 0; i < model->layerCount - 1; i++)
  {
    ...
    xMatrix *weightMatrix = xMatrix_new(model->neuronCounts[i + 1], model->neuronCounts[i]);
    ...
    xList_append(weightMatrices, weightMatrix);
    ...
    if (biasMatrices != NULL)
    {
      ...
      {
        xMatrix_set(biasMatrix, 0, j, model->biasValues[i * model->neuronCounts[i + 1] + j]);
      }
      xList_append(biasMatrices, biasMatrix);
    }
    ...
    {
      fprintf(stderr, "FNN Loader: Failed to allocate memory for activation function\n");
      return -1;
    }
    *activationFunction = model->activationFunctions[i];
    xList_append(activationFunctions, activationFunction);
  }
