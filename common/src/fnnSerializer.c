                    ...
                    FnnActivation_e activationFunction)
{
  ...
  uint32_t *neuronCounts =
      (uint32_t *)realloc(model->neuronCounts, (model->layerCount + 1) * sizeof(uint32_t));
  ...
  if (model->layerCount > 0)
  {
    ...
    FnnActivation_e *activationFunctions = (FnnActivation_e *)realloc(
        model->activationFunctions, model->layerCount * sizeof(FnnActivation_e));
    ...
    float *biases =
        (float *)realloc(model->biasValues, (model->totalBiases + neuronCount) * sizeof(float));
    if (biases == NULL)
    {
...
int32_t fnnSerialize(const char *filename, FnnModel *model)
{
  ...
  uint64_t expectedSize = sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint64_t) +
                          sizeof(uint64_t) + sizeof(uint32_t) +
                          model->layerCount * sizeof(uint32_t) +
                          (model->layerCount - 1) * sizeof(FnnActivation_e) +
  ...
  wrSize +=
      fwrite(model->neuronCounts, sizeof(uint32_t), model->layerCount, file) * sizeof(uint32_t);
  wrSize +=
      fwrite(model->activationFunctions, sizeof(FnnActivation_e), (model->layerCount - 1), file) *
      sizeof(FnnActivation_e);
...
FnnModel *fnnDeserialize(const char *filename)
{
  ...
  // read model header
  if (fread(&model->magic, sizeof(uint32_t), 1, file) != 1 ||
      fread(&model->version, sizeof(uint16_t), 1, file) != 1 ||
      fread(&model->totalWeights, sizeof(uint64_t), 1, file) != 1 ||
      fread(&model->totalBiases, sizeof(uint64_t), 1, file) != 1 ||
  ...
  model->activationFunctions =
      (FnnActivation_e *)malloc((model->layerCount - 1) * sizeof(FnnActivation_e));
  ...
  if (fread(model->neuronCounts, sizeof(uint32_t), model->layerCount, file) != model->layerCount ||
      fread(model->activationFunctions, sizeof(FnnActivation_e), model->layerCount - 1, file) !=
          model->layerCount - 1)
  {
